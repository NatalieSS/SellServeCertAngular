/**
 * Main AngularJS logic
 */
angular.module('sell_serve_cert_app', [
  // "SSC.LandingPage",
  // 'sell_serve_cert_app.factories',
  'ngRoute'
])
...
.config(['$routeProvider', '$locationProvider', function ($routeProvider, $locationProvider) {
  $routeProvider
    ...
    // Pages
    .when("/how_it_works", {templateUrl: "partials/how_it_works.html", controller: "PageCtrl"})
    .when("/pick_a_state_view", {templateUrl: "partials/pick_a_state_view.html", controller: "PageCtrl"})
    .when("/state_view", {templateUrl: "partials/state_view.html", controller: "PageCtrl"})
    // else 404
...
.controller('PageCtrl', function ($scope, $http, $location) {
  ...
  //create userInput var for use when user chooses a state
  $scope.userInput = "";

  // save all data needed for our landing page scope object
  $scope.data = {
  model: null,
  states: [
...
// Functionality for changing view to state_view 
// (saves userInput to variable before changing view)
  $scope.changeView = function(view) {
        
    $scope.userInput = $scope.data.model;
    console.log("before: ", $scope.userInput);

    $location.path(view);
    console.log("after: ", $scope.userInput);
  };

  // Allows responsive dropdown menu for mobile
  $scope.clickMenu = function() {
    $(".icon").click(function () {
      var x = document.getElementById("nav");
      if (x.className === "topnav navigation") {
          x.className += " responsive";
      } else {
          x.className = "topnav navigation";
      }
    });
  }
...
// Scrollbar Functionality - Modified code from this carousel: http://codepen.io/carimu/pen/jbgQvy
var startScroll = function() {
  ;(function(factory){
     ...
     * Creates a new DOM element and tests existence of properties on its
...
//////////////// Bottom "Find your course and Sign Up" Button Functionality:
// when this form is submitted, redirect to another state-specific info view. 
// (Functionality is like an SPA but only with simple JS/jQuery code.)//////////

  // $(".btn-text").click(function(event) {
  //   event.preventDefault();

  //   $(".pick-a-state-page").removeClass("hidden");
